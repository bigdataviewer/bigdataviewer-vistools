...
public class EllipsoidsOverlay implements OverlayRenderer
{
	private final AffineTransform3D sourceTransform;

	private final ArrayList< Ellipsoid > ellipsoids;
	...
	public void setEllipsoids( final List< Ellipsoid > ellipsoids )
	{
		this.ellipsoids.clear();
		this.ellipsoids.addAll( ellipsoids );
	}
	...
	public void drawOverlays( final Graphics g )
	{
		...
		if ( t >= ellipsoids.size() )
			return;

		final Ellipsoid ellipsoid = ellipsoids.get( t );
	...
	private void updateNormals( final Ellipsoid ellipsoid )
	{
		if ( ellipsoid != null )
		{
			normals.clear();
